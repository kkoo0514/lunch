    ...
    <div class="modal-backdrop" @click.self="close">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td colspan="3"><textarea v-model="form.memo" style="width:100%;"></textarea></td>
        </tr>
...
const openModal = async (id) => {
  const response = await axios.get('http://localhost:8000/getPlaceById'+id )
  form.id = id
  ...
  form.memo = response.data.result.memo
}

defineExpose({ openModal });

const save = async() => {
  form.surveyYear = form.date.split('-')[0]
  form.surveyMonth = parseInt( form.date.split('-')[1] )
  form.surveyDay = parseInt( form.date.split('-')[2] )
  try {
    const response = await axios.put('/api/v1/water/'+form.recordId,form)
